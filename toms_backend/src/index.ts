...
const dbPath = path.join(__dirname, '..', 'toms_backend.db');
...
function initDb() {
  db.serialize(() => {
    ...
    db.run('ALTER TABLE route_stops ADD COLUMN path_id INTEGER', () => {
      ...
      db.all('SELECT id FROM routes', [], (err, routes: any[]) => {
        if (!err && routes) {
          routes.forEach(r => {
            db.get('SELECT id FROM route_paths WHERE route_id = ?', [r.id], (err, row: any) => {
              if (!row) {
                db.run('INSERT INTO route_paths (route_id, name, color) VALUES (?, ?, ?)', [r.id, 'Primary', '#00d2ff'], function(this: sqlite3.RunResult) {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS conductors (
      ...
      db.all("PRAGMA table_info(conductors)", (err, rows) => {
        if (!err && rows) {
          ...
          if (!columns.includes('contact_number')) db.run("ALTER TABLE conductors ADD COLUMN contact_number TEXT");
          if (!columns.includes('address')) db.run("ALTER TABLE conductors ADD COLUMN address TEXT");
        }
      });
      // Insert a default admin/test conductor if none exists
      db.get('SELECT COUNT(*) as count FROM conductors', (err, row: any) => {
        if (row && row.count === 0) {
          db.run('INSERT INTO conductors (username, password_hash, name, contact_number, address, assigned_vehicle) VALUES (?, ?, ?, ?, ?, ?)', 
            ['test1', hashPassword('password123'), 'John Doe', '09123456789', 'Main St, Iligan City', 'BUS-101']);
        }
      });
    });
    ...
    db.run(`CREATE TABLE IF NOT EXISTS vehicles (
      ...
      db.all("PRAGMA table_info(vehicles)", (err, rows) => {
        if (!err && rows) {
          const columns = rows.map((r: any) => r.name);
          if (!columns.includes('assigned_route_id')) db.run("ALTER TABLE vehicles ADD COLUMN assigned_route_id INTEGER");
          if (!columns.includes('assigned_conductor_id')) db.run("ALTER TABLE vehicles ADD COLUMN assigned_conductor_id INTEGER");
        }
      });
    ...
    db.run(`CREATE TABLE IF NOT EXISTS events (
      ...
      db.all("PRAGMA table_info(events)", (err, rows) => {
        if (!err && rows) {
          const columns = rows.map((r: any) => r.name);
          if (!columns.includes('conductor_id')) db.run("ALTER TABLE events ADD COLUMN conductor_id TEXT");
          if (!columns.includes('conductor_name')) db.run("ALTER TABLE events ADD COLUMN conductor_name TEXT");
          if (!columns.includes('current_lat')) db.run("ALTER TABLE events ADD COLUMN current_lat REAL");
          if (!columns.includes('current_lon')) db.run("ALTER TABLE events ADD COLUMN current_lon REAL");
        }
      });
    });

    // Insert a default route if none exists
    db.get('SELECT COUNT(*) as count FROM routes', (err: Error | null, row: any) => {
      if (row.count === 0) {
        db.run('INSERT INTO routes (name, company_id) VALUES (?, ?)', ['Default Route', 'company_1'], function(this: sqlite3.RunResult, err: Error | null) {
          if (!err) {
...
app.get('/api/routes', (req, res) => {
  db.all('SELECT id, name, company_id, base_fare, per_km_fare FROM routes ORDER BY id ASC', [], (err: Error | null, rows: any[]) => {
    if (err) {
...
app.post('/api/routes', (req, res) => {
  ...
  db.run('INSERT INTO routes (name, company_id) VALUES (?, ?)', [name, company_id || 'company_1'], function(this: sqlite3.RunResult, err: Error | null) {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
...
app.get('/api/routes/:id/paths', (req, res) => {
  ...
  db.all('SELECT id, name, color FROM route_paths WHERE route_id = ? ORDER BY id ASC', [routeId], (err, paths: any[]) => {
    if (err) return res.status(500).json({ error: err.message });
    ...
    db.all('SELECT * FROM route_schedules WHERE route_id = ?', [routeId], (err, schedules: any[]) => {
      ...
      paths.forEach(p => {
        ...
        db.all('SELECT id, name, lat, lon, stop_order, radius_m FROM route_stops WHERE path_id = ? ORDER BY stop_order ASC', [p.id], (err, stops: any[]) => {
        p.stops = (stops || []).map((r: any) => ({
          id: r.id,
          name: r.name,
          lat: r.lat,
          lon: r.lon,
...
app.post('/api/routes/:id/paths', (req, res) => {
  ...
  db.run('INSERT INTO route_paths (route_id, name, color) VALUES (?, ?, ?)', [routeId, name || 'Alternative', color || '#2ed573'], function(this: sqlite3.RunResult, err: Error | null) {
    if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/routes/:id/schedules', (req, res) => {
  ...
  if (!Array.isArray(schedules)) return res.status(400).json({ error: 'Expected array of schedules' });
...
app.post('/api/conductors/login', (req, res) => {
  ...
  db.get('SELECT id, username, name, company_id, assigned_vehicle FROM conductors WHERE username = ? AND password_hash = ?', 
    [username, hashed], 
    (err, row: any) => {
      if (err) return res.status(500).json({ error: err.message });
...
app.post('/api/conductors', (req, res) => {
  ...
  db.run(
    'INSERT INTO conductors (username, password_hash, name, contact_number, address, assigned_vehicle) VALUES (?, ?, ?, ?, ?, ?)',
    [username, hash, name, contact_number || null, address || null, assigned_vehicle || null],
    function(err) {
      if (err) {
...
app.post('/api/vehicles', (req, res) => {
  ...
  if (!id || !plate_number) return res.status(400).json({ error: 'id and plate_number are required' });
  
  db.run(
    'INSERT INTO vehicles (id, plate_number, max_capacity, status) VALUES (?, ?, ?, ?)',
    [id, plate_number, max_capacity || 20, status || 'Active'],
    function(err) {
      if (err) return res.status(500).json({ error: err.message });
...
app.put('/api/vehicles/:id', (req, res) => {
  ...
  db.serialize(() => {
    ...
    db.run(
      'UPDATE vehicles SET plate_number = COALESCE(?, plate_number), max_capacity = COALESCE(?, max_capacity), status = COALESCE(?, status), assigned_route_id = ?, assigned_conductor_id = ? WHERE id = ?',
      [plate_number, max_capacity, status, assigned_route_id || null, assigned_conductor_id || null, id],
      function(err) {
        if (err) {
          db.run('ROLLBACK');
          return res.status(500).json({ error: err.message });
...
app.get('/api/routes/:id/stops', (req, res) => {
  const routeId = req.params.id;
  db.all('SELECT id, name, lat, lon, stop_order, radius_m FROM route_stops WHERE route_id = ? ORDER BY stop_order ASC', [routeId], (err: Error | null, rows: any[]) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
    // Map to expected format
    const stops = rows.map((r: any) => ({
      id: r.id,
      name: r.name,
      lat: r.lat,
      lon: r.lon,
...
app.post('/api/events', (req, res) => {
  ...
  db.serialize(() => {
    ...
    let latestFleetStatus: Record<string, any> = {};
...
app.get('/api/fleet/status', (req, res) => {
  ...
  db.all(query, [`${todayStr}%`], (err: Error | null, rows: any[]) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
...
app.get('/api/audit/logs', (req, res) => {
  ...
  db.get(countQuery, params, (errCount: Error | null, countRow: any) => {
    if (errCount) {
    ...
    db.all(dataQuery, dataParams, (errData: Error | null, rows: any[]) => {
      if (errData) {
...
app.get('/api/analytics/revenue', (req, res) => {
  ...
  db.all(query, [startDateStr], (err: Error | null, rows: any[]) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
...
app.get('/api/conductors', (req, res) => {
  ...
  db.all(query, [todayStr], (err: Error | null, eventRows: any[]) => {
    if (err) return res.status(500).json({ error: err.message });
